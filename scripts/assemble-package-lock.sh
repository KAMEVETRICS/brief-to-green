...
if compgen -G "$PARTS_DIR/c*.txt" > /dev/null; then
  cat "$PARTS_DIR"/c*.txt > "$OUT"
elif compgen -G "$PARTS_DIR/a*.txt" > /dev/null && compgen -G "$PARTS_DIR/b*.txt" > /dev/null; then
  ...
  echo "No lock parts under $PARTS_DIR" >&2
